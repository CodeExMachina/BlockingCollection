...
namespace code_machina {
    ...
    ///
    /// In addition, it implements support for attaching and detaching workers to the condition variable.
    ...
    template<typename ThreadContainerType, typename SignalStrategyType, typename ConditionVarType, typename LockType>
    class ConditionVariable
    {
    public:

        /// Initializes a new instance of the ConditionVariable class without an upper-bound.
        ConditionVariable()
        ...
        /// active workers are workers that are currently NOT waiting on this condition variable.
        ...
        /// If the worker was not previously registered then this method has no effect.
        /// @see Attach
        void detach() {
        ...
        void signal() {
            ...
            // issue a signal only when there are no active workers, or when the count
            // starts to grow beyond a threshold level
            if(signal_.should_signal(active_workers_, total_workers_, item_count_, bounded_capacity_)) {
                increment_active();
                ConditionVarTraits<ConditionVarType, LockType>::signal(condition_var_);
            }
        }
        ...
        template<class Rep, class Period> bool wait_for(std::unique_lock<LockType>& lock, const std::chrono::duration<Rep, Period>& rel_time) {

            decrement_active();

            bool timed_out = ConditionVarTraits<ConditionVarType, LockType>::wait_for(condition_var_, lock, rel_time);
    ...
    ///
    /// A strategy object for determining whether or not a "not empty" condition variable should issue a signal.
    ///
    /// This strategy will only return true if there are no active workers (i.e. all workers are waiting
    /// on empty BlockingCollection). Or when the BlockingCollection's element count starts to grow beyond a
    /// threshold level.
    ///
    ...
    ///
    /// A strategy object for determining whether or not a "not full" condition variable should issue a signal.
    ///
    /// This strategy will only return true if there are no active workers (i.e. all workers are
    /// waiting on a full BlockingCollection). Or when the BlockingCollection's available capacity
    /// starts to grow beyond a threshold level.
    ///
    /// This approach minimizes condition variable sleeps, wakes and lock contention. Which in turn,
    /// improves performance and makes it more predictable.
    ...
    template<typename ThreadContainerType, typename NotFullSignalStrategy, typename NotEmptySignalStrategy, typename ConditionVarType, typename LockType> struct ConditionVariableGenerator {
    ...
    namespace detail {
        ...
        public:
            ...
            /// @param bounded_capacity The max number of elements this container can hold.
            ...
            /// @returns True if the element was added successfully; otherwise, false.
            ...
            /// @param [out] item When this method returns, if the element was removed and returned successfully, item
            /// contains the removed element. If no element was available to be removed, the value is unspecified.
        ...
        private:
            ...
            template <typename... Args> bool try_emplace_i(Args&&... args, std::false_type) {
                container_.emplace_front(std::forward<Args>(args)...);
                return true;
            }

            template <typename... Args> bool try_emplace_i(Args&&... args, std::true_type) {
    ...
    public:
        ...
        /// If any method in BlockingCollection is executing while the size property is being accessd, the return value
        /// is approximate. size may reflect a number that is either greater than or less than the actual number of
        ...
        /// Tries to add the given element value to the BlockingCollection<T> within the specified time period.
        /// Value is moved into the new element.
        /// @param value the value of the element to try to add
        /// @param rel_time An object of type std::chrono::duration representing the maximum time to spend waiting.
        /// @return A BlockCollectionStatus code.
        ...
        BlockingCollectionStatus try_add_timed(U&& value, const std::chrono::duration<Rep, Period>& rel_time) {
        ...
        /// If a bounded capacity was specified when this instance of BlockingCollection<T> was initialized,
        /// a call to Emplace may block until space is available to store the provided item.
        /// @param args arguments to forward to the constructor of the element
        /// @return A BlockCollectionStatus code.
        /// @see BlockingCollectionStatus
        template<typename... Args>
        BlockingCollectionStatus emplace(Args&&... args) {
            try_emplace_timed(std::chrono::milliseconds(-1), std::forward<Args>(args)...);
        }
        ...
        BlockingCollectionStatus try_emplace_timed(const std::chrono::duration<Rep, Period>& rel_time, Args&&... args) {
            {
        ...
        /// @see BlockingCollectionStatus
        /// @see http://en.cppreference.com/w/cpp/chrono/duration
        template<class Rep, class Period> BlockingCollectionStatus try_take(T& item, const std::chrono::duration<Rep, Period>& rel_time) {
            {
                std::unique_lock<LockType> guard(lock_);
        ...
        /// Use std::make_move_iterator if the elements should be moved instead of copied.
        ...
        /// @return A BlockCollectionStatus code.
        /// @see BlockingCollectionStatus
        template <typename Iterator> BlockingCollectionStatus add_bulk(Iterator first, Iterator last, size_t& added) {
        ...
        /// @return A BlockCollectionStatus code.
        /// @see BlockingCollectionStatus
        template <typename Iterator> BlockingCollectionStatus try_add_bulk(Iterator first, Iterator last, size_t& added) {
        ...
        template<typename Iterator, class Rep, class Period> BlockingCollectionStatus try_add_bulk(Iterator first, Iterator last, size_t& added, const std::chrono::duration<Rep, Period>& rel_time) {
            {
                added = 0;
        ...
        /// A call to take_bulk may block until an element is available to be removed.
        /// Use std::make_move_iterator if the elements should be moved instead of copied.
        ...
        /// @return A BlockCollectionStatus code.
        /// @see BlockingCollectionStatus
        template <typename Iterator> BlockingCollectionStatus take_bulk(Iterator first, size_t count, size_t& taken) {
            return try_take_bulk(first, count, taken, std::chrono::milliseconds(-1));
        }
        
        /// Takes up to count elements from the BlockingCollection<T>.
        /// If the collection is empty, this method immediately returns without taking any items.
        /// Use std::make_move_iterator if the elements should be moved instead of copied.
        /// @param[out] first Contains first item taken.
        /// @param count The number of elements to take from collection.
        /// @param[out] taken The actual number of elements taken.
        /// @return A BlockCollectionStatus code.
        /// @see BlockingCollectionStatus
        template <typename Iterator> BlockingCollectionStatus try_take_bulk(Iterator first, size_t count, size_t& taken) {
            return try_take_bulk(first, count, taken, std::chrono::milliseconds::zero());
        }
        
        /// Tries to take up to count elements from the BlockingCollection<T> within
        /// the specified time period.
        /// If the collection is empty, this method immediately returns without taking any items.
        /// Use std::make_move_iterator if the elements should be moved instead of copied.
        /// @param[out] first Contains first item taken.
        ...
        /// @see BlockingCollectionStatus
        /// @see http://en.cppreference.com/w/cpp/chrono/durations
        template <typename Iterator, class Rep, class Period> BlockingCollectionStatus try_take_bulk(Iterator first, size_t count, size_t& taken, const std::chrono::duration<Rep, Period>& rel_time) {
            {
                taken = 0;
    ...
    public:
        ...
        : collection_(collection), status_(BlockingCollectionStatus::Ok), wait_for_first_item(true) {
        }

        Iterator(BlockingCollection<T, ContainerType, ConditionVariableGenerator> &collection, BlockingCollectionStatus status)
        ...
        bool operator!=(const Iterator& it)  {
            if (wait_for_first_item) {
                ...
                status_ = collection_.try_take(item_, std::chrono::milliseconds(-1)); // -1 forces TryTake to wait
            }
    ...
    public:
        ...
        Iterator end()   { return { *this, BlockingCollectionStatus::Completed }; }
    ...
    protected:
        ...
        /// @see BlockingCollectionStatus
        /// @see http://en.cppreference.com/w/cpp/chrono/duration
        template<class Rep, class Period> BlockingCollectionStatus wait_not_full_condition(std::unique_lock<LockType>& lock, const std::chrono::duration<Rep, Period>& rel_time) {
        ...
        /// @see BlockingCollectionStatus
        /// @see http://en.cppreference.com/w/cpp/chrono/duration
        template<class Rep, class Period> BlockingCollectionStatus wait_not_empty_condition(std::unique_lock<LockType>& lock, const std::chrono::duration<Rep, Period>& rel_time) {
    ...
    private:
        ...
        LockType lock_; ///< Synchronizes access to the BlockCollection.
        ContainerType container_; ///< The underlying Container (e.g. Queue, Stack).
    };
    ...
    public:
        ...
        /// Attempts to add an element to the collection according to the element's priority.
        /// This new element is constructed in place using args as the arguments for its construction.
        /// @param args Arguments forwarded to construct the new element.
        template <typename... Args> bool try_emplace(Args&&... args) {
            if(container_.size() == bounded_capacity_)
    ...
    public:
        ...
        /// @see BlockingCollectionStatus
        /// @see http://en.cppreference.com/w/cpp/chrono/duration
        template<class Rep, class Period> BlockingCollectionStatus try_take_prio(T& item, const std::chrono::duration<Rep, Period>& rel_time) {
            {
        ...
        /// Takes up to count low priority elements from the PriorityBlockingCollection<T>.
        /// A call to take_prio_bulk may block until an element is available to be removed.
        /// Use std::make_move_iterator if the elements should be moved instead of copied.
        /// @param[out] first Contains first item taken.
        /// @param count The number of elements to take from collection.
        /// @param[out] taken The actual number of elements taken.
        /// @return A BlockCollectionStatus code.
        /// @see BlockingCollectionStatus
        template<typename Iterator> BlockingCollectionStatus take_prio_bulk(Iterator first, size_t count, size_t& taken) {
        ...
        /// @return A BlockCollectionStatus code.
        /// @see BlockingCollectionStatus
        template<typename Iterator> BlockingCollectionStatus try_take_prio_bulk(Iterator first, size_t count, size_t& taken) {
            return try_take_prio_bulk(first, count, taken, std::chrono::milliseconds::zero());
        }
        
        /// Tries to take up to count low priority elements from the PriorityBlockingCollection<T> within
        /// the specified time period.
        /// If the collection is empty, this method immediately returns without taking any items.
        /// Use std::make_move_iterator if the elements should be moved instead of copied.
        /// @param[out] first Contains first item taken.
        ...
        /// @see BlockingCollectionStatus
        /// @see http://en.cppreference.com/w/cpp/chrono/durations
        template <typename Iterator, class Rep, class Period> BlockingCollectionStatus try_take_prio_bulk(Iterator first, size_t count, size_t& taken, const std::chrono::duration<Rep, Period>& rel_time) {
            {
                taken = 0;
    ...
    namespace detail {
        ...
        /// @tparam BlockingCollectionType The type of BlockingCollection to Guard.
        ...
        template<typename BlockingCollectionType, typename GuardType> class Guard {
        public:
